  ...
  return (
    <section className="flex flex-col items-start justify-between gap-8 w-full">
      <div className="flex flex-col gap-2">
        <h1 className="fade-in text-cinder-800 font-medium dark:font-normal text-4xl dark:text-macaroni-and-cheese-300 font-ebgaramond">
          {article.title}
        </h1>
        <div
      ...
      {article.category === "mock-test" && (
        ...
        >
          ...
          <div className="grid gap-2 text-macaroni-and-cheese-800 dark:text-cinder-300">
            <p className="">
              I recommend attempting the problems before proceeding to read the
              article. You can download the problems and the answer key using
              the buttons below.
            </p>
          </div>
          <div className="flex gap-2 justify-between sm:justify-start pt-2">
            <DownloadButton type="problems" slug={article.slug} />
            {article.answerKey && (
              <DownloadButton type="answer-key" slug={article.slug} />
            )}
          </div>
        </div>
      )}
